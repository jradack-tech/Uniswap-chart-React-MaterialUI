...
var prev_mint_id, prev_burn_id, isExtraAdd = false, isSearched = false, isSearched1 = false;
var flag = true, interval_flag = true;
var myInterval_5;
...
function Uniswapexplorer (props) {
	...
	if (selected_pair.data && (selected_pair.data.pairs.length !== 0)) {
		...
		if (pair_info.data && pair_info.data.pairs.length !== 0) {
			...
			console.log("hahaha", selected_data)
				...
				if (pair_data.token1.symbol === "WETH" && (pair_data.token0.symbol === "USDC" || pair_data.token0.symbol === "USDT" || pair_data.token0.symbol === "DAI")) {
			        setToken0(pair_data.token1);
			        setToken1(pair_data.token0);
			        runTransactions(pair_data.token1.id, pair_data.token0.id);
                	...
                	setToken0(pair_data.token0);
			        setToken1(pair_data.token1);
			        runTransactions(pair_data.token0.id, pair_data.token1.id);
	...
	var list = (
		<Fragment>
			{(searchedPairs.length !== 0) && searchedPairs.sort((a, b) => b.txCount - a.txCount).map((pair, i) => (
				<li id={pair.id} key={i}>
					<a id={pair.id} href="#" onClick={onClick}>
						<ImageComponent 
							id={pair.id}
			          ...
			          <div className="input-group">
			            ...
			            <div className="input-group-append ">
			              <button type="button" className="btn btn-search">
			              	{loadingSearch ? 
			                	<i className="fa fa-spinner fa-spin" style={{fontSize:"15px", color: "#F6EEEE"}}></i> : 
			                  ...
			                  <div className="tab-content">
			                	...
			                	{/* Display Token Name/Symbol 
			                      ...
			                      <div className="price-box">
			                        ...
			                        <div className="price-right">
			                          <p>${(token0 && ethPrice.data) && (parseFloat(token0.derivedETH) * parseFloat(ethPrice.data.bundles[0].ethPrice)) }</p>
			                        </div>
			                      </div>
			        ...
			        <div className="row">
			          <div className="col-lg-4">
			              ...
			              <ul className="mt-4 token-info-list card">
			                ...
			                <li>
			                  <div className="total-wrp">
			                    <div className="uni-name">
			                    	<span> 
				                    	<b>{(token0 && ethPrice) && token0.symbol }
				                    		<small>({(token0 && ethPrice) && token0.name})</small> 
				                    	</b>
			                    	</span>
			                    </div>
			                ...
			                <li>
			                  <div className="total-wrp">
			                    ...
			                    <div className="left-content-per">
			                      <h3>
			                      $ {(token0 && ethPrice.data) && (parseFloat(token0.derivedETH) * parseFloat(ethPrice.data.bundles[0].ethPrice))}
			                      </h3>
			                    </div>
			                  </div>
			                </li>
			                <li className="my-1 data-volume">
			                  ...
			                  ><span className="data-volume-right"> 
			                  	$ {(token0 && ethPrice.data) && (parseFloat(token0.derivedETH) * parseFloat(ethPrice.data.bundles[0].ethPrice) * parseFloat(selected_data.totalSupply)).toFixed(3) }
			                  </span>
			                </li>
			                <li className="my-1 data-volume">
			                  <span>Circulating supply : </span
			                  ><span className="data-volume-right"> 
			                  	{(token0 && ethPrice.data) && parseFloat(selected_data.totalSupply) }
			                  </span>
			                </li>
			            {/*
			            	<li className="my-1 data-volume">
			            ...
			            */}
			                ...
			                <li className="my-1 data-volume">
			                  <span>Pool {(token0 && selected_data.token0)? token0.symbol: ""} : </span
			                  ><span className="data-volume-right"> 
			                  		{(token1 && selected_data.token0) && parseFloat(reserve0).toFixed(2)}
			                  </span>
			                </li>
			                <li className="my-1 data-volume">
			                  <span>Pooled {(token1 && selected_data.token1)? token1.symbol: ""} : </span
			                  ><span className="data-volume-right">
			                  		{(token0 && selected_data.token1) && parseFloat(reserve1).toFixed(2)}
			                  </span>
			                </li>
				                      ...
				                      >
				                        <thead>
										  <tr>								 
				                            ...
				                            <th>Type</th>
				                            <th>Price(USD)</th>
				                            <th>Amount {token0 && selected_data.token0 && token0.symbol}</th>
				                            <th>Amount {token1 && selected_data.token0 && token1.symbol}</th>
				                            <th>Maker</th>
				                          </tr>
				                        </thead>
										<tbody>
					                          {token0 && selected_data.token0 && txs.length !== 0 && 
					                        	txs.map((t, k) => (
					                        		<tr key={k}>
							                            <td>
							                            ...
							                            <td>
															{t.buyCurrency.symbol === token0.symbol ?
																<span className="sellcol">SELL</span> : 
																<span className="buycol">BUY</span> 
															}
														</td>
							                            <td>${ (t.tradeAmount / t.baseAmount).toFixed(4) }</td>
							                            <td>{ t.buyCurrency.symbol === token0.symbol ? t.buyAmount.toFixed(4) : t.sellAmount.toFixed(4) }</td>
							                            <td>{ t.buyCurrency.symbol !== token0.symbol ? t.buyAmount.toFixed(4) : t.sellAmount.toFixed(4)}</td>
							                            <td><a target="_blank" href={"https://bscscan.com/address/"+t.transaction.txFrom.address}>{t.transaction.txFrom.address.substr(0,15)}...</a></td>
							                        </tr>
					                        	))
					                       	  }
